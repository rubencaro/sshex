defmodule SSHEx.ConfigurableClientKeys do
  ...
  `
  cb_module = SSHEx.ConfigurableClientKeys.get_cb_module(keyfile: "path/to/keyfile", known_hosts_file: "path_to_known_hostsFile", accept_hosts: false)
  SSHEx.connect(
      ip: to_charlist(hostname), 
      user: to_charlist(username), 
      key_cb: cb_module
  ...
  def get_cb_module(opts) do
    opts = 
      opts
      |> Keyword.update!(:key_file, &File.open!/1)
      |> Keyword.update!(:known_hosts_file, &File.open!/1)      
    {__MODULE__, opts}
  end
...
# SSHEx.doConnect("ec2-54-202-189-109.us-west-2.compute.amazonaws.com", "sashavoynow")